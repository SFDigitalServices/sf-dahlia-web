...
describe("Account Settings", () => {
  it("runs through the account settings page", () => {
    ...
    cy.get("#firstName-error").should("contain.text", "Enter first name")
    ...
    // Submit DOB change
    cy.get("input#dobObject\\.birthMonth").clear()
    cy.get("input#dobObject\\.birthMonth").type("10")
    cy.get("input#dobObject\\.birthDay").clear()
    cy.get("input#dobObject\\.birthDay").type("12")
    cy.get("input#dobObject\\.birthYear").clear()
    cy.get("input#dobObject\\.birthYear").type("2000")
    cy.intercept(
      "/api/v1/account/update",
    ...
    // Create DOB error
    cy.get("input#dobObject\\.birthYear").clear()
    cy.get("input#dobObject\\.birthYear").type("1899")
    ...
    cy.get("button").contains("Enter email address").click()
    cy.get("button").contains("Enter email address").type("t")
    cy.contains("Email missing @ symbol. Enter email like: example@web.com")
    cy.get("button").contains("Email missing @ symbol").click()
    cy.get("button").contains("Email missing @ symbol").type("est@")
    cy.contains("Email entered incorrectly. Enter email like: example@web.com")
    cy.get("button").contains("Email entered incorrectly").click()
    cy.get("button").contains("Email entered incorrectly").type("test")
    cy.contains("Email missing a dot ‘.’ in the domain. Enter email like: example@web.com")
    cy.get("button").contains("Email missing a dot ‘.’ in the domain").click()
    cy.get("button").contains("Email missing a dot ‘.’ in the domain").type(".com")
    ...
    cy.get("button").contains("Enter current password").click()
    cy.get("button").contains("Enter current password").type("password")
    cy.get('input[name="password"]').click()
    cy.get('input[name="currentPassword"]').click()
    cy.get("button").contains("Enter new password").click()
    cy.get("button").contains("Enter new password").type("password")
    cy.contains("Choose a strong password with at least 8 characters, 1 letter, and 1 number")
    cy.get("button").contains("Choose a strong password").click()
    cy.get("button").contains("Choose a strong password").type("1")
