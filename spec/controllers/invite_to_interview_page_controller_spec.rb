...
RSpec.describe InviteToInterviewPageController do
  ...
  describe '#index' do
    context 'with valid parameters' do
      ...
      it 'renders the invite_to_interview template' do
        expect(response).to render_template('invite_to_interview')
      end

      it 'sets the invite_to_interview_props instance variable' do
        expect(assigns(:invite_to_interview_props)).to eq({
    ...
    context 'with json web tokens' do
      ...
      it 'creates a token for the preview link' do
        ...
        expect(assigns(:invite_to_interview_props)).to have_key(:submitPreviewLinkTokenParam)
      end
  ...
  describe '#documents' do
    before do
      get :documents, params: { id: listing_id }
    end
    ...
    it 'sets documentsPath to true' do
      expect(assigns(:invite_to_interview_props)).to eq({
                                                         assetPaths: { logo: 'logo.png' },
                                                         documentsPath: true,
                                                       })
